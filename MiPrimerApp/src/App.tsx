...
const response: Response = {
  ...
  listaCategoria: [
    ...
    {
      nombre:'OFERTAS DEL DIA',
    },
    {
      nombre:'LO MAS VENDIDO',
    },
    {
      nombre:'LO MAS BUSCADO',
    }
  ]
}

function App() {
  const { listaProducto, listaCategoria } = response;
  const [carrito, setCarrito] = useState<Post[]>([]);
  const [contProduct, setContProduct] = useState(0);

  const [filtroCheck, setFiltroCheck] = useState(false);
  const [buscar, setBuscar] = useState('');
  const [filtro, setFiltro] = useState<Post[]>([]);
  ...
  const handleOnCheckbox = (e: { target: { value: string; checked: unknown } }) => {
    setFiltroSelect({
      ...filtroSelect,
    ...
    if (e.target.checked){
      if (e.target.value==='de0a1000'){
        setFiltroCheck(true);
        const res = listaProducto.posts.filter((posts) => posts.precio > 1000);
        console.log(listaProducto);
        console.log(res);
        setFiltro([...filtro, ...res]);
      }

      else {setFiltroCheck(true);
      const res = listaProducto.posts.filter((posts) => posts.categoria === e.target.value);
      setFiltro([...filtro, ...res]);}
      
    }

    else {
      setFiltroCheck(false)
      const res = filtro.filter((posts) => posts.categoria !== e.target.value);
      setFiltro([...res]);
    };
      ...
      <div className='checkbox-container'>
        ...
        <div className='input-checkbox'>
          ...
          <input 
            onChange={handleOnCheckbox}
            name='filtro'
            value='OFERTAS DEL DIA'
            type="checkbox"
            id='ofertasDelDia'
          />
          <label htmlFor='ofertasDelDia'>ofertasDelDia</label>
          <input 
            onChange={handleOnCheckbox}
            name='filtro'
            value='LO MAS VENDIDO'
            type="checkbox"
            id='loMasVendido'
          />
          <label htmlFor='loMasVendido'>loMasVendido</label>
          <input 
            onChange={handleOnCheckbox}
            name='filtro'
            value='LO MAS BUSCADO'
            type="checkbox"
            id='loMasBuscado'
          />
          <label htmlFor='loMasBuscado'>loMasBuscado</label>
        </div>
      </div>
       {!filtroCheck ? (
          buscar === '' ? (
            listaCategoria.map((categoria) => (
              <ProductCardContainer NombreCategoria={categoria.nombre} key={categoria.nombre}>
                <ProductList
                  productos={listaProducto.posts.filter(
                    (post) => post.categoria === categoria.nombre
                  )}
                  carrito={carrito}
                  setCarrito={setCarrito}
                  contProduct={contProduct}
            ...
            <ProductCardContainer NombreCategoria='Productos filtrados'>
              <ProductList
                productos={listaProducto.posts.filter((post) =>
                  post.nombre.toLowerCase().includes(buscar.toLowerCase())
                )}
                carrito={carrito}
                setCarrito={setCarrito}
                contProduct={contProduct}
          ...
          <ProductCardContainer NombreCategoria='Productos filtrados'>
            <ProductList
              productos={listaProducto.posts.filter((post) => filtro.includes(post))}
              carrito={carrito}
              setCarrito={setCarrito}
              contProduct={contProduct}
              setContProduct={setContProduct}
